const SEAL_CONFIG = {
  ...
  glyphs: [
    { text: "天", x: 47, y: 24, mobileX: 45, mobileY: 22 },
    { text: "罡", x: 72, y: 48, mobileX: 82, mobileY: 45 },
    { text: "镇", x: 28, y: 55, mobileX: 18, mobileY: 56 },
    { text: "邪", x: 58, y: 78, mobileX: 59, mobileY: 80 },
  ],
};
...
glyphNodes.forEach((node) => {
  node.addEventListener("click", () => discoverGlyph(node));
});

scene.addEventListener("click", (event) => {
  if (!(event.target instanceof Element) || !event.target.closest(".seal-glyph")) {
    triggerMistake();
  }
});
